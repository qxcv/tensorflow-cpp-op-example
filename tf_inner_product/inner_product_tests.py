...
from tf_inner_product.inner_product import inner_product
...
class InnerProductOpTest(unittest.TestCase):
    def test_raisesExceptionWithIncompatibleDimensions(self):
        with tf.Session(''):
            with self.assertRaises(ValueError):
                inner_product([1, 2], [[1, 2], [3, 4]]).eval()
            with self.assertRaises(ValueError):
                self.assertRaises(
                    inner_product([1, 2], [1, 2, 3, 4]).eval(), ValueError)
            with self.assertRaises(ValueError):
                self.assertRaises(
                    inner_product([1, 2, 3], [[1, 2], [3, 4]]).eval(),
                    ValueError)

    def test_innerProductHardCoded(self):
        with tf.Session(''):
            result = inner_product([[1], [2]], [[1, 2], [3, 4]]).eval()
    ...
    def test_innerProductGradientXHardCoded(self):
        with tf.Session('') as sess:
            ...
            Wx_inner_product = inner_product(tf.reshape(x, [-1, 1]), W)

            grad_x_tf = tf.gradients(Wx_tf, x)
            grad_x_inner_product = tf.gradients(Wx_inner_product, x)

            gradient_tf = sess.run(
                grad_x_tf,
                feed_dict={x: np.asarray([1, 2]).astype(np.float32)})
            gradient_inner_product = sess.run(
                grad_x_inner_product,
                feed_dict={x: np.asarray([1, 2]).astype(np.float32)})
    ...
    def test_innerProductGradientWHardCoded(self):
        with tf.Session('') as sess:
            ...
            gradient_tf = sess.run(
                grad_W_tf,
                feed_dict={W: np.asarray([[1, 2], [3, 4]]).astype(np.float32)})
            gradient_inner_product = sess.run(
                grad_W_inner_product,
                feed_dict={W: np.asarray([[1, 2], [3, 4]]).astype(np.float32)})

            self.assertEqual(gradient_tf[0][0][0],
                             gradient_inner_product[0][0][0])
            self.assertEqual(gradient_tf[0][0][1],
                             gradient_inner_product[0][0][1])
            self.assertEqual(gradient_tf[0][1][0],
                             gradient_inner_product[0][1][0])
            self.assertEqual(gradient_tf[0][1][1],
                             gradient_inner_product[0][1][1])
    ...
    def test_innerProductRandom(self):
        with tf.Session(''):
            ...
            for i in range(100):
                ...
                result = inner_product(x_rand, W_rand).eval()
    ...
    def test_innerProductGradientXRandom(self):
        with tf.Session('') as sess:
            ...
            for i in range(100):
                ...
                gradient_tf = sess.run(grad_x_tf,
                                       feed_dict={
                                           x: x_rand,
                                           W: W_rand
                                       })
                gradient_inner_product = sess.run(grad_x_inner_product,
                                                  feed_dict={
                                                      x: x_rand,
                                                      W: W_rand
                                                  })

                np.testing.assert_array_equal(gradient_tf,
    ...
    def test_innerProductGradientWRandom(self):
        with tf.Session('') as sess:
            ...
            for i in range(100):
                ...
                gradient_tf = sess.run(grad_W_tf,
                                       feed_dict={
                                           x: x_rand,
                                           W: W_rand
                                       })
                gradient_inner_product = sess.run(grad_W_inner_product,
                                                  feed_dict={
                                                      x: x_rand,
                                                      W: W_rand
                                                  })
